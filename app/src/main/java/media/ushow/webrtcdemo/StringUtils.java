...
//        return ret;
    ...
    // 万  粉丝，关注 w
    public static String intForStrWan(int number, String appendStr) {
        StringBuilder formatMoney = new StringBuilder();
        if (number >= 10000) {
            NumberFormat nf = new DecimalFormat("0.##");
            nf.setRoundingMode(RoundingMode.FLOOR);
            formatMoney.append(nf.format(number / 10000));
            if (!TextUtils.isEmpty(appendStr)) {
                formatMoney.append(appendStr);
            }
        } else {
            NumberFormat nf = new DecimalFormat("0.##");
            formatMoney.append(nf.format(number));
        }
    ...
    //单位是分-- 转元
    public static String fenIntFormatMoney2Point(int money) {
        String formatMoney;
        double value1 = 0;
        try {
            value1 = money / 100.00;
...
//        return s_yf + s_rq+"-"+s_hu;//10月29日－10:00:00
    ...
    public static String fullUrl(String url) {
        String baseUrl = "";//AppDef.getWebServerAddress();
        String fullUrl;
        try {
            //
            if (url.startsWith("http")) {
    ...
    //拼接http地址
    public static String fullApiUrl(String url) {
        String baseUrl = "";//AppDef.getAPIServerAddress();
        String fullUrl;
        try {
            //
            if (url.startsWith("http")) {
    ...
    public static String videoThumbUrl(String url) {
        return fullApiUrl(url)+"?x-oss-process=video/snapshot,t_0";
    }
...
//            lastClickTime = time;
    ...
    /**
     * 1万以下的直接展示
     * 1万-100万，保留1个小数点，如1.1万
     * 100万（含）以上不要小数点
     */
    public static String getCountByWanSpecial(String src) {
        try {
